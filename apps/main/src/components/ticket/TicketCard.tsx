...
export default function TicketCard({ data }: TicketCardProps) {
  return (
    <Link href={`tickets/${data.eventName}/${data.tokenId}`} className="relative w-full ">
      <article className="relative flex w-full justify-between px-2 py-4 transition-all ease-in-out hover:rounded-lg hover:border-[#EBF8FF] hover:scale-110 hover:bg-[#EBF8FF] ">
        <Image
